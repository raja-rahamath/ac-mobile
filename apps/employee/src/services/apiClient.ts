...
export const STORAGE_KEYS = {
  ACCESS_TOKEN: '@agentcare_employee_access_token',
  REFRESH_TOKEN: '@agentcare_employee_refresh_token',
  USER: '@agentcare_employee_user',
};
...
// For file uploads (photos, etc.)
export async function uploadFile<T = any>(
  endpoint: string,
  formData: FormData,
  options: Omit<FetchOptions, 'body'> = {}
): Promise<T> {
  const { requiresAuth = true, baseUrl = API_CONFIG.BASE_URL } = options;
  ...
  if (!response.ok) {
    ...
    const errorMessage = errorData.error?.message || errorData.error || errorData.message || 'Upload failed';
    throw new Error(typeof errorMessage === 'string' ? errorMessage : JSON.stringify(errorMessage));
  }
...
export const api = {
  ...
  upload: <T = any>(endpoint: string, formData: FormData, options?: Omit<FetchOptions, 'body'>) =>
